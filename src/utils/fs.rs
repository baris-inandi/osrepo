...
pub fn expand_tilde<P: AsRef<Path>>(path_user_input: P) -> Result<PathBuf, std::io::Error> {
    ...
    return dirs::home_dir()
        .map(|mut h| {
            if h == Path::new("/") {
                p.strip_prefix("~")
                    .expect("utils::fs::expand_tilde: invlid path")
                    .to_path_buf()
            } else {
                h.push(
                    p.strip_prefix("~/")
...
// Returns File object after creating tile-expanded filepath
pub fn expand_create_file(filepath: &str) -> Result<std::fs::File, std::io::Error> {
    return std::fs::File::create(expand_tilde(filepath)?);
}
